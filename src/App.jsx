...
import { 
  ...
  Briefcase, Menu, X, Save, Lock, Award, AlertTriangle, Trash2, ArrowRight
} from 'lucide-react';
import { 
...
const StatCard = ({ title, value, remark, icon: Icon, trend, trendValue, isCenterMobile = false }) => (
  <div className={`bg-[#170b0c] border border-[#2b1416] rounded-2xl p-4 md:p-6 relative overflow-hidden group h-full flex flex-col justify-between ${isCenterMobile ? 'w-[60%] mx-auto md:w-full md:mx-0' : 'w-full'}`}>
    ...
    <div className="relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2 mb-2">
        <div className="text-xl md:text-3xl font-bold text-white leading-tight">{value}</div>
        {remark && <span className="text-[10px] md:text-sm font-bold text-[#ff2a44]">{remark}</span>}
      </div>
      <div className="flex items-center text-[10px] md:text-xs">
        {trend === 'up' ? (
          <TrendingUp size={14} className="text-emerald-400 mr-1 drop-shadow-[0_0_5px_rgba(52,211,153,0.6)] shrink-0" />
        ) : (
          <TrendingDown size={14} className="text-red-500 mr-1 drop-shadow-[0_0_5px_rgba(239,68,68,0.6)] shrink-0" />
        )}
        ...
        <span className="text-[#a38c8e] ml-1 truncate">dari bulan lepas</span>
      </div>
    </div>
  </div>
);
...
export default function App() {
  ...
  const [plans, setPlans] = useState([
    { id: 1, date: '1-Jul-2026', symbol: 'QES', setup: 'Pullback + Vol', entry: '0.545', sl: '0.525', tp: '0.610', riskPct: '3', status: 'ACTIVE', lockedData: null }
  ]);

  const [newPlan, setNewPlan] = useState({
    symbol: '', setup: setupsList[0] || '', entry: '', sl: '', tp: '', riskPct: '2'
  });
  ...
  const handleAddPlan = () => {
    if (!newPlan.symbol || !newPlan.entry) return;
    const dateStr = new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }).replace(/ /g, '-');
    setPlans([{ ...newPlan, id: Date.now(), date: dateStr, status: 'WATCHLIST', lockedData: null }, ...plans]);
    setNewPlan({ symbol: '', setup: setupsList[0] || '', entry: '', sl: '', tp: '', riskPct: '2' });
  };
  ...
  const calculatePosition = (row, refCapital) => {
    ...
    let rrRatio = 0;
    if (entry > 0 && tp > entry && riskPerUnit > 0) {
        rrRatio = (tp - entry) / riskPerUnit;
    }
    return { maxLoss, unitsToBuy, capitalNeeded, rrRatio };
  };
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4">
        ...
        <div className="col-span-2 lg:col-span-1 flex justify-center lg:block">
           <StatCard title="Jumlah ROI" value="19.00%" icon={Percent} trend="up" trendValue="+4.2%" isCenterMobile={true} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        <div className="lg:col-span-2 bg-[#170b0c] border border-[#2b1416] rounded-2xl p-4 md:p-6 flex flex-col">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div><h2 className="text-base md:text-lg font-bold text-white">Analitik Prestasi Bulanan</h2><p className="text-[#a38c8e] text-xs mt-1">Aliran Keuntungan Bersih (RM) untuk tahun 2026</p></div>
          ...
          <div className="flex-1 min-h-[250px] md:min-h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <defs><linearGradient id="colorProfit" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#ff2a44" stopOpacity={0.4}/><stop offset="95%" stopColor="#ff2a44" stopOpacity={0}/></linearGradient></defs>
        ...
        <div className="bg-[#170b0c] border border-[#2b1416] rounded-2xl p-4 md:p-6 flex flex-col min-h-[280px]">
          <h2 className="text-sm font-bold text-white">Pecahan Strategi</h2><p className="text-[#a38c8e] text-xs mb-4">Prestasi mengikut setup (Win Rate %)</p>
          <div className="flex-1 flex flex-col items-center justify-center relative min-h-[150px]">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none"><span className="text-2xl font-bold text-white">68%</span></div>
          </div>
          <div className="mt-4 space-y-2">
            {strategyData.map((item, index) => (
              <div key={index} className="flex justify-between items-center text-xs">
                <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full" style={{ backgroundColor: COLORS[index] }}></div><span className="text-[#a38c8e]">{item.name}</span></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        <div className="lg:col-span-2 bg-[#170b0c] border border-[#2b1416] rounded-2xl p-4 md:p-6 flex flex-col">
          <div className="flex justify-between items-center mb-6"><h2 className="text-base md:text-lg font-bold">Trade Terkini</h2><button onClick={() => navigateTo('journal')} className="text-xs md:text-sm text-[#ff2a44] hover:text-white transition-colors">Lihat Semua</button></div>
          <div className="overflow-x-auto flex-1">
            <table className="w-full text-left border-collapse whitespace-nowrap min-w-[500px]">
              <thead>
                <tr className="border-b border-[#2b1416] text-[#a38c8e] text-xs uppercase tracking-wider"><th className="pb-4 pl-2 font-medium">Saham</th><th className="pb-4 font-medium">Strategi</th><th className="pb-4 font-medium">Entry</th><th className="pb-4 font-medium">P&L</th><th className="pb-4 font-medium text-right pr-2">Status</th></tr>
              </thead>
        ...
        <div className="bg-[#170b0c] border border-[#2b1416] rounded-2xl p-4 md:p-6 flex flex-col min-h-[280px]">
          <h2 className="text-sm font-bold text-white">Kekerapan Sektor</h2><p className="text-[#a38c8e] text-xs mb-4">Sektor Bursa yang aktif di-trade (%)</p>
          <div className="flex-1 flex flex-col items-center justify-center relative min-h-[150px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={sectorData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                <RechartsTooltip cursor={{fill: '#2b1416'}} contentStyle={{ backgroundColor: '#170b0c', borderColor: '#ff2a44', borderRadius: '8px', color: '#fff', fontSize: '12px' }} itemStyle={{ color: '#ff2a44', fontWeight: 'bold' }} />
                <Bar dataKey="value" radius={[6, 6, 0, 0]} maxBarSize={40}>
                  {sectorData.map((entry, index) => <Cell key={`cell-${index}`} fill={SECTOR_COLORS[index % SECTOR_COLORS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div className="mt-4 space-y-2">
            {sectorData.map((item, index) => (
              <div key={index} className="flex justify-between items-center text-xs">
                <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full" style={{ backgroundColor: SECTOR_COLORS[index] }}></div><span className="text-[#a38c8e] truncate max-w-[120px]">{item.name}</span></div>
                <span className="text-white font-bold">{item.value}%</span>
              </div>
            ))}
          </div>
  ...
  const renderTradingPlan = () => {
    // Live Calculation untuk Input Baru
    const liveCalc = calculatePosition(newPlan, capital);
    
    return (
      <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
        
        {/* SECTION 1: KALKULATOR SETUP (Borang Input) */}
        <div className="bg-[#170b0c] border border-[#2b1416] rounded-2xl overflow-hidden shadow-2xl">
          <div className="bg-gradient-to-r from-[#2b1416] to-[#170b0c] p-4 md:p-6 border-b border-[#2b1416] flex justify-between items-center">
            <div>
               <h2 className="text-lg md:text-xl font-bold text-white flex items-center gap-2"><Target size={20} className="text-[#ff2a44]"/> Kalkulator Setup Baru</h2>
               <p className="text-xs text-[#a38c8e] mt-1">Masukkan parameter untuk kira Position Sizing.</p>
            </div>
            <div className="text-right hidden sm:block">
               <p className="text-[10px] text-[#a38c8e] uppercase">Modal Rujukan</p>
               <p className="text-lg font-bold text-white font-mono">RM {capital.toLocaleString()}</p>
            </div>
          </div>
          ...
          <div className="p-4 md:p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5"><label className="text-[10px] md:text-xs text-[#a38c8e] uppercase font-semibold">Simbol Saham</label><input type="text" name="symbol" value={newPlan.symbol} onChange={handleNewPlanChange} placeholder="Cth: YTL" className="w-full bg-[#090505] border border-[#2b1416] rounded-xl p-3 text-sm text-white uppercase outline-none focus:border-[#ff2a44] transition-colors" /></div>
                <div className="space-y-1.5"><label className="text-[10px] md:text-xs text-[#a38c8e] uppercase font-semibold">Strategi</label><select name="setup" value={newPlan.setup} onChange={handleNewPlanChange} className="w-full bg-[#090505] border border-[#2b1416] rounded-xl p-3 text-sm text-white outline-none focus:border-[#ff2a44] transition-colors">{setupsList.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
              </div>
              <div className="grid grid-cols-3 gap-3 md:gap-4">
                <div className="space-y-1.5"><label className="text-[10px] md:text-xs text-[#a38c8e] uppercase font-semibold">Entry</label><input type="number" step="0.005" name="entry" value={newPlan.entry} onChange={handleNewPlanChange} placeholder="0.00" className="w-full bg-[#090505] border border-[#2b1416] rounded-xl p-3 text-sm text-blue-300 outline-none focus:border-blue-500 font-mono" /></div>
                <div className="space-y-1.5"><label className="text-[10px] md:text-xs text-[#a38c8e] uppercase font-semibold">Stop Loss</label><input type="number" step="0.005" name="sl" value={newPlan.sl} onChange={handleNewPlanChange} placeholder="0.00" className="w-full bg-[#090505] border border-[#2b1416] rounded-xl p-3 text-sm text-red-400 outline-none focus:border-red-500 font-mono" /></div>
                <div className="space-y-1.5"><label className="text-[10px] md:text-xs text-[#a38c8e] uppercase font-semibold">Target (TP)</label><input type="number" step="0.005" name="tp" value={newPlan.tp} onChange={handleNewPlanChange} placeholder="0.00" className="w-full bg-[#090505] border border-[#2b1416] rounded-xl p-3 text-sm text-emerald-400 outline-none focus:border-emerald-500 font-mono" /></div>
              </div>
              <div className="w-1/2 pr-2 space-y-1.5">
                <label className="text-[10px] md:text-xs text-[#a38c8e] uppercase font-semibold">Risiko (%)</label>
                <div className="relative">
                  <input type="number" name="riskPct" value={newPlan.riskPct} onChange={handleNewPlanChange} className="w-full bg-[#090505] border border-[#2b1416] rounded-xl p-3 pl-8 text-sm text-white outline-none focus:border-[#ff2a44] font-mono" />
            ...
            {/* Live Results Panel */}
            <div className="bg-[#090505] border border-[#2b1416] rounded-xl p-5 flex flex-col justify-between">
              <div className="space-y-4">
                <div className="flex justify-between items-end border-b border-[#2b1416] pb-3">
                  <p className="text-xs text-[#a38c8e]">Unit Boleh Beli</p>
                  <p className="text-2xl font-bold font-mono text-blue-400">{liveCalc.unitsToBuy > 0 ? liveCalc.unitsToBuy.toLocaleString() : '0'}</p>
                </div>
                <div className="flex justify-between items-end border-b border-[#2b1416] pb-3">
                  <p className="text-xs text-[#a38c8e]">Modal Diperlukan</p>
                  <p className="text-lg font-bold font-mono text-white">RM {liveCalc.capitalNeeded > 0 ? liveCalc.capitalNeeded.toLocaleString(undefined, {minimumFractionDigits: 2}) : '0.00'}</p>
                </div>
                <div className="flex justify-between items-end">
                  <p className="text-xs text-[#a38c8e]">Nisbah R:R</p>
                  <p className={`text-lg font-bold font-mono ${liveCalc.rrRatio >= 2 ? 'text-emerald-400' : liveCalc.rrRatio > 0 ? 'text-yellow-400' : 'text-[#a38c8e]'}`}>
                    {liveCalc.rrRatio > 0 ? `1 : ${liveCalc.rrRatio.toFixed(2)}` : '-'}
                  </p>
                </div>
              </div>
              <button onClick={handleAddPlan} className="w-full mt-6 bg-[#ff2a44] text-white py-3 flex items-center justify-center gap-2 rounded-xl font-bold hover:bg-[#e0253b] transition-colors shadow-[0_0_15px_rgba(255,42,68,0.3)]">
                <Save size={18} /> Simpan Pelan
              </button>
            </div>
          </div>
        </div>

        {/* SECTION 2: SENARAI KAD PELAN */}
        <div>
          <div className="flex items-center gap-2 mb-4">
            <BookOpen size={18} className="text-[#a38c8e]" />
            <h2 className="text-base md:text-lg font-bold text-white">Senarai Pantauan & Aktif</h2>
          </div>
          
          {plans.length === 0 ? (
            <div className="text-center py-10 bg-[#170b0c] border border-[#2b1416] border-dashed rounded-2xl text-[#a38c8e]">
              Tiada pelan disimpan. Sila masukkan data di atas.
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {plans.map(plan => {
                const calc = calculatePosition(plan, capital);
                const isLocked = plan.status === 'EXECUTED';
                return (
                  <div key={plan.id} className={`bg-[#170b0c] border border-[#2b1416] rounded-2xl p-5 flex flex-col relative transition-all hover:border-[#4a2528] ${isLocked ? 'opacity-80' : ''}`}>
                    
                    {/* Card Header */}
                    <div className="flex justify-between items-start mb-4 border-b border-[#2b1416] pb-3">
                      <div>
                        <h3 className="text-xl font-bold text-white flex items-center gap-2">
                          {isLocked && <Lock size={14} className="text-[#a38c8e]" />} {plan.symbol}
                        </h3>
                        <p className="text-[10px] text-[#a38c8e] uppercase tracking-wider">{plan.setup} • {plan.date}</p>
                      </div>
                      <select 
                        value={plan.status} 
                        onChange={(e) => handleRowStatusChange(plan.id, e.target.value)}
                        className={`text-[9px] font-bold px-3 py-1.5 rounded-full outline-none cursor-pointer border appearance-none text-center
                          ${plan.status === 'ACTIVE' ? 'bg-blue-900/30 text-blue-400 border-blue-500/30' : 
                            plan.status === 'WATCHLIST' ? 'bg-yellow-900/30 text-yellow-400 border-yellow-500/30' : 
                            plan.status === 'EXECUTED' ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/30' : 
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-4">
                      <div className="bg-[#090505] p-2 rounded-lg text-center border border-[#2b1416]">
                        <p className="text-[9px] text-[#a38c8e] mb-1">ENTRY</p>
                        <p className="text-sm font-mono text-blue-300">{parseFloat(plan.entry).toFixed(3)}</p>
                      </div>
                      <div className="bg-[#090505] p-2 rounded-lg text-center border border-[#2b1416]">
                        <p className="text-[9px] text-[#a38c8e] mb-1">STOP LOSS</p>
                        <p className="text-sm font-mono text-red-400">{parseFloat(plan.sl).toFixed(3)}</p>
                      </div>
                      <div className="bg-[#090505] p-2 rounded-lg text-center border border-[#2b1416]">
                        <p className="text-[9px] text-[#a38c8e] mb-1">TARGET</p>
                        <p className="text-sm font-mono text-emerald-400">{plan.tp ? parseFloat(plan.tp).toFixed(3) : '-'}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 space-y-2 mb-4">
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-[#a38c8e]">Risk & R:R</span>
                        <span className="font-mono font-semibold text-white">RM {calc.maxLoss.toFixed(2)} <span className="text-[#4a2528] mx-1">|</span> <span className={calc.rrRatio >= 2 ? 'text-emerald-400' : 'text-yellow-400'}>1:{calc.rrRatio.toFixed(2)}</span></span>
                      </div>
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-[#a38c8e]">Unit & Modal</span>
                        <span className="font-mono font-semibold text-white">{calc.unitsToBuy} <span className="text-[#4a2528] mx-1">|</span> RM {calc.capitalNeeded.toFixed(2)}</span>
                      </div>
                    </div>

                    {/* Card Action */}
                    <div className="pt-3 border-t border-[#2b1416] flex justify-end">
                      <button onClick={() => handleDeletePlan(plan.id)} className="text-[#a38c8e] hover:text-red-500 hover:bg-red-900/20 p-2 rounded-lg transition-colors">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden w-full">
        <header className="h-16 md:h-20 border-b border-[#2b1416] flex items-center justify-between px-4 md:px-8 bg-[#090505]/80 backdrop-blur-md sticky top-0 z-10 shrink-0">
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <div className="relative flex-1 md:w-72 max-w-[200px] md:max-w-none"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#a38c8e]" /><input type="text" placeholder="Cari..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-[#170b0c] border border-[#2b1416] rounded-full py-2 pl-9 md:pl-10 pr-4 text-xs md:text-sm text-white placeholder-[#a38c8e] focus:outline-none focus:border-[#ff2a44] transition-colors uppercase" /></div>
            
            {/* Butang [+] Mobile Sahaja (Disusun sebelah Search Bar) */}
            <button onClick={() => navigateTo('plan')} className="md:hidden w-8 h-8 md:w-9 md:h-9 bg-transparent shrink-0 flex items-center justify-center transition-transform active:scale-95">
              <Plus size={28} strokeWidth={3} className="text-white drop-shadow-[0_0_8px_rgba(255,42,68,0.8)]" />
            </button>
          </div>
          <div className="flex items-center gap-3 md:gap-6 ml-2">
            <button onClick={() => navigateTo('plan')} className="hidden md:flex items-center gap-2 bg-[#ff2a44] text-white px-3 md:px-4 py-2 rounded-lg text-xs md:text-sm font-bold shadow-[0_0_15px_rgba(255,42,68,0.3)] hover:bg-[#e0253b] transition-all"><Plus size={16} /> <span className="hidden md:inline">Trade Baru</span></button>
